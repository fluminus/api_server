...
defmodule PeriodicTask do
  ...
  defp pn(auth, state) do
    case Fluminus.API.api(auth,
      ...
      {:ok, resp} ->
        ...
        {:noreply, %{auth: %Authorization{idsrv: idsrv, jwt: jwt, user_id: state.auth.user_id}, count: state.count+1}}
            ...
            {:ok, renewed_auth} ->
              ...
              case SQL.query(FluminusServer.Repo, query_string, []) do
                {:ok, _} ->
                  ...
                  pn(renewed_auth, state)

                _ -> :error
              end
            {:error, reason} ->
              Logger.warn("Failed to renew JWT for #{state.auth.user_id}, reason: " <> Kernel.inspect(reason))
              auth
          end
        else
          pn(auth, state)
